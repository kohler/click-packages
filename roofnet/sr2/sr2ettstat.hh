...
public:
  int _rate;
  int _size;
  SR2RateSize(int r, int s): _rate(r), _size(s) { };
  
  inline bool
  operator==(SR2RateSize other)
  {
    return (other._rate == _rate && other._size == _size);
  }
};

static const uint8_t _sr2_ett_version = 0x04;

class SR2ETTStat : public Element {

public:

  // Packet formats:

  // SR2ETTStat packets have a link_probe header immediately following
...
public:
  ...
  class SR2ETTMetric *_ett_metric;
  class SR2TXCountMetric *_etx_metric;
...
public:
  ...
  void update_link(IPAddress from, IPAddress to, Vector<SR2RateSize> rs, Vector<int> fwd, Vector<int> rev, uint32_t seq);
  void send_probe();
 ...
 public:
  ...
  const char *class_name() const		{ return "SR2ETTStat"; }
